        ...
        async with httpx.AsyncClient(
            ...
            response = await client.post(
                ...
                json={
                    ...
                    "jobid": request["jobId"]  #todo: this is a temporary code to test the promptflow and will be replaced with the actual jobid from UI
                },
                headers=headers,
            )
        resp = response.json()
...
async def conversation():
    logging.debug("Received /conversation request") #todo: jtang to remove this
    if not request.is_json:
        return jsonify({"error": "request must be json"}), 415
    request_json = await request.get_json()
    logging.debug(f"Request body: {request_json}") #todo: jtang to remove this
    return await conversation_internal(request_json)


@bp.route("/frontend_settings", methods=["GET"])
def get_frontend_settings():
    logging.debug("Received /frontend_settings request") #todo: jtang to remove this
    try:
...
async def add_conversation():
    logging.debug("Received /history/generate request") #todo: jtang to remove this
    ...
    try:
        ...
        logging.debug(f"Request body 1: {request_body}")  #todo: jtang to remove this
        history_metadata["conversation_id"] = conversation_id
        request_body["history_metadata"] = history_metadata
        logging.debug(f"Request body 2: {request_body}")  #todo: jtang to remove this
...
async def update_conversation():
    logging.debug("Received /history/update request") #todo: jtang to remove this
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
...
async def update_message():
    logging.debug("Received /history/message_feedback request") #todo: jtang to remove this
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
    cosmos_conversation_client = init_cosmosdb_client()
...
async def delete_conversation():
    logging.debug("Received /history/delete request") #todo: jtang to remove this
...
async def list_conversations():
    logging.debug("Received /history/list request") #todo: jtang to remove this
...
async def get_conversation():
    logging.debug("Received /history/read request")  #todo: jtang to remove this
    authenticated_user = get_authenticated_user_details(request_headers=request.headers)
    user_id = authenticated_user["user_principal_id"]
